...
def recent_episodes():
    ...
    return jsonify({'currentPage': page, 'hasNextPage': False, 'results': results})
...
def watch_episode():
    ...
    if not data: return jsonify({'error': 'Episode not found', 'url': episode_url}), 404
    sources = [{'url': l['url'], 'isM3U8': '.m3u8' in l['url'].lower(), 'quality': l['quality']} for l in data.get('download_links', [])]
    return jsonify({
        'headers': {'Referer': 'https://witanime.you/', 'User-Agent': 'Mozilla/5.0...'},
        'sources': sources,
